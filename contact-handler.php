<?php
// Ensure no errors or warnings are output before JSON
...
try {
    ...
    $user_agent = isset($_SERVER['HTTP_USER_AGENT']) ? substr($_SERVER['HTTP_USER_AGENT'], 0, 50) . '...' : 'unknown';
    ...
    if (!empty($errors)) {
        ...
        exit; // Make sure to terminate after sending response
    }
    ...
    @file_put_contents($log_file, "$timestamp - [$request_id] Exception: " . $e->getMessage() . "\n", FILE_APPEND);
...
// Ensure no additional output after the JSON
exit; 